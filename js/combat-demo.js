...
class CombatDemo {
  constructor() {
    ...
    this.enemyPool = [
      { name: "Goblin Sentry", rank: "Minion", hp: 45, maxHp: 45, atk: 7, armor: 2, ward: 0, sprite: "assets/enemies/anim/Goblin.gif", intent: "Strikes for 7 Physical DMG" },
      { name: "Gnome Tinkerer", rank: "Normal", hp: 55, maxHp: 55, atk: 8, armor: 3, ward: 4, sprite: "assets/enemies/anim/Gnome.gif", intent: "Tinkers strike for 8 DMG" },
      { name: "Bandit Marauder", rank: "Aggressive", hp: 70, maxHp: 70, atk: 10, armor: 4, ward: 0, sprite: "assets/enemies/anim/Bandit.gif", intent: "Heavy slash for 10 DMG" },
      { name: "Cultist Pyromancer", rank: "Caster", hp: 80, maxHp: 80, atk: 12, armor: 0, ward: 10, sprite: "assets/enemies/anim/Cultist.gif", intent: "Pyromancy blast for 12 Magic DMG" },
      { name: "Elite Knight", rank: "Elite", hp: 130, maxHp: 130, atk: 14, armor: 10, ward: 4, sprite: "assets/enemies/anim/EliteKnight.gif", intent: "Shield bash for 14 Physical DMG" },
      { name: "Bone Amalgam", rank: "Act Boss", hp: 220, maxHp: 220, atk: 16, armor: 14, ward: 0, sprite: "assets/enemies/anim/EliteKnight.gif", intent: "Bone pulverize for 16 Physical DMG" }
    ];

    this.heroAnimMap = {
      'Deck Monk': 'assets/heroes/anim/DeckMonk.gif',
      'Demonling': 'assets/heroes/anim/Demonling.gif',
      'Gorilla Gabe': 'assets/heroes/anim/GorillaGabe.gif',
      'Hunter The Hedgehog': 'assets/heroes/anim/Hunter the Hedgehog.gif',
      'Necri': 'assets/heroes/anim/Necri.gif',
      'Lost Spirit': 'assets/heroes/anim/lostspirit.gif',
      'Scrap Goblin': 'assets/heroes/anim/Goblin.gif'
    };

    const urlParams = new URLSearchParams(window.location.search);
    this.useCustomDeck = urlParams.get('customDeck') === '1';
  ...
  bindEvents() {
    ...
    // Deck Source Dropdown (Standard vs Custom)
    const deckSourceSelect = document.getElementById('combat-deck-source-select');
    if (deckSourceSelect) {
      deckSourceSelect.value = this.useCustomDeck ? 'custom' : 'standard';
      deckSourceSelect.addEventListener('change', (e) => {
        this.useCustomDeck = e.target.value === 'custom';
        if (this.useCustomDeck) {
          const raw = localStorage.getItem('decked_custom_deck');
          if (!raw || Object.keys(JSON.parse(raw || '{}')).length === 0) {
            if (window.showToast) {
              window.showToast('ℹ️ No saved deck found in Deck Builder. Using standard cards.', 'info');
            }
          }
        }
        this.resetCombat();
  ...
  buildCombatDeck() {
    ...
    // Check if custom deck is selected and available
    if (this.useCustomDeck) {
      try {
        const raw = localStorage.getItem('decked_custom_deck');
        if (raw) {
          const deckDict = JSON.parse(raw);
          const instances = [];
          Object.entries(deckDict).forEach(([cardId, count]) => {
          ...
          if (instances.length >= 10) {
            ...
            this.deck = instances;
            this.combatLog.push(`🎴 Loaded custom deck with ${this.deck.length} cards from Deck Builder.`);
            return;
          }
        }
      } catch (e) {
  ...
  renderCombatState() {
    ...
    const heroSprite = (this.heroAnimMap && this.heroAnimMap[this.selectedHero.name]) || this.selectedHero.sprite;
    if (playerAvatarImg && heroSprite) playerAvatarImg.src = heroSprite;
